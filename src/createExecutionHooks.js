...
export const createExecutionHooks = ({ listenMacro, listenMicro, getNano }) => {
  ...
  if (listenMacro) {
    ...
    const addDelayedMacro = (fn, ms, ...args) => {
      const addedMs = nano.getMillisecond()
      return addMacro(() => fn(...args), () => nano.getMillisecond() - addedMs >= ms)
    }
    ...
    listenMacro(() => {
      ...
      if (nano.lowerThan(fakeNano)) {
        ellapsedMs = fakeNano.minus(nano).getMillisecond()
      } else {
        ellapsedMs = nano.minus(fakeNano).getMillisecond()
      }
      nano = fakeNano
      runMacros()
    })
